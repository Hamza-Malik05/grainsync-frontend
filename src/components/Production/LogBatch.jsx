...
export default function LogBatch() {
  ...
  const [filteredProductStorageUnits, setFilteredProductStorageUnits] = useState([]);
  ...
  const fetchEmployees = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/employees/production`);
  ...
  const fetchStorageUnits = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/storage-units`);
  ...
  const fetchProductStorageUnits = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/product-inventory-storage/product-inventory`);
  ...
  const handleInputChange = (e) => {
    ...
    if ((name === "quantity_used" || name === "quantity_produced") && value < 0) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-2">Raw Material Storage Unit</label>
                <select
